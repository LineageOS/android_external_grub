...
get_vendor_ramdisk_fragment_info_content(grub_device_t device) {
  char *buf_list_none = NULL, *buf_list_platform = NULL,
       *buf_list_recovery = NULL, *buf_list_dlkm = NULL,
       *dest_buf __attribute__((unused)) = NULL;
  struct vendor_ramdisk_fragment_entry *vrfe, *vrfe_head;

  buf_list_none = grub_strdup(" ");
  buf_list_platform = grub_strdup(" ");
  buf_list_recovery = grub_strdup(" ");
  buf_list_dlkm = grub_strdup(" ");
  ...
  while (vrfe) {
    ...
    /* FIXME
        ...
        local_strappend(dest_buf, ")/");
        local_strappend(dest_buf, get_vendor_ramdisk_fragment_filename(vrfe));
        local_strappend(dest_buf, " ");
    */
    vrfe = vrfe->next;
  }
  ...
  return grub_xasprintf(VENDOR_COMPONENT_VENDOR_RAMDISK_FRAGMENT_INFO_FMT,
...
android_bootimg_dir(grub_device_t device,
  ...
  if (check_boot_img_header(device)) {
    ...
    while (vrfe) {
      grub_memset(&info, 0, sizeof(info));
      hook(get_vendor_ramdisk_fragment_filename(vrfe), &info, hook_data);
      vrfe = vrfe->next;
    }
    if (vrfe_head)
      free_vendor_ramdisk_fragment_entries(vrfe_head);
...
android_bootimg_open(grub_file_t file, const char* path) {
  ...
  if (check_boot_img_header(file->device)) {
    ...
    while (vrfe) {
      if (!grub_strcmp(path + 1, get_vendor_ramdisk_fragment_filename(vrfe))) {
